                    ...
                    {THEMES.map((t) => {
                        ...
                        return (
                            <button
                                ...
                                className={`relative ${swatchSize} rounded-full border transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] ${active ? 'scale-110 border-white' : 'border-gray-600 hover:scale-105'
                                    }`}
                                style={{ backgroundImage: `linear-gradient(135deg, ${colors.from}, ${colors.to})` }}
                            >
                                {active && (
                                    <span className={`absolute -top-0.5 -right-0.5 inline-flex ${checkmarkSize} items-center justify-center rounded-full bg-white text-gray-900`}>✓</span>
                                )}
                            </button>
                        )
                    })}
