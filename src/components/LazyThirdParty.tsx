	...
	useEffect(() => {
		...
		async function init() {
			try {
				...
				setMods({
					Analytics: Analytics as unknown as ComponentType<unknown>,
					SpeedInsights: SpeedInsights as unknown as ComponentType<unknown>,
				});
			} catch (err) {
				if (process.env.NODE_ENV === "development")
					console.error("LazyThirdParty failed to load", err);
			}
		}
