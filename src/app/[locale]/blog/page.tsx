...
export default async function BlogPage({ params }: BlogPageProps) {
  ...
  const rssTitleId = `rss-title-${locale}`
  const jsonTitleId = `json-title-${locale}`
  const arrowTitleId = `arrow-title-${locale}`
          ...
          <div className="max-w-4xl mx-auto text-center">
            <FadeInUp delay={0.2}>
              <h1 className="text-4xl md:text-6xl font-bold text-white mb-6">
                {t('hero.title')}{' '}
                <span className="text-accent-gradient">{t('hero.titleHighlight')}</span>
              </h1>
            </FadeInUp>
            <FadeInUp delay={0.4}>
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                >
                  <svg
                    ...
                    className="h-4 w-4"
                    role="img"
                    aria-labelledby={rssTitleId}
                  >
                    <title id={rssTitleId}>RSS Feed</title>
                ...
                >
                  ...
                  >
                    <title id={jsonTitleId}>JSON Feed</title>
            ...
            {posts.length === 0 ? (
              <ScrollReveal>
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-300 mb-8">{t('comingSoon.description')}</p>
                  <Link
                    href="/"
                  ...
                  <StaggerItem key={post.slug}>
                    <article className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg p-6 hover:border-purple-500 transition-colors group">
                      <div className="flex items-center justify-between text-sm text-gray-400 mb-3">
                        ...
                        <span>
                          {post.readingTime} •{' '}
                          {post.wordCount.toLocaleString(locale === 'fr' ? 'fr-FR' : 'en-US')}{' '}
                          {t('post.words')}
                        </span>
                      </div>
                      ...
                      >
                        ...
                        <svg
                          ...
                          viewBox="0 0 24 24"
                          role="img"
                          aria-labelledby={arrowTitleId}
                        >
                          <title id={arrowTitleId}>Read more</title>
                          <path
                            strokeLinecap="round"
            ...
            {totalPages > 1 && (
              <div className="mt-12 flex items-center justify-center gap-4">
                <span className="text-gray-400 text-sm">
                  {t('pagination.page')} 1 {t('pagination.of')} {totalPages}
                </span>
                <Link
