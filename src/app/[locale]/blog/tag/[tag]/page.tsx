...
export async function generateStaticParams() {
  ...
  return [
    ...Array.from(enTags).map((tag) => ({
      locale: "en",
      tag: encodeURIComponent(tag),
    })),
    ...Array.from(frTags).map((tag) => ({
      locale: "fr",
      tag: encodeURIComponent(tag),
    })),
  ];
...
export default async function TagPage({ params }: TagPageProps) {
  ...
  const backTagTitleId = `back-tag-title-${locale}-${tag}`;
  const readArrowTitleId = `read-arrow-title-${locale}-${tag}`;
              ...
              <div className="mb-6">
                ...
                >
                  <svg
                    ...
                    viewBox="0 0 24 24"
                    role="img"
                    aria-labelledby={backTagTitleId}
                  >
                    <title id={backTagTitleId}>Back to blog</title>
                    <path
                      strokeLinecap="round"
                  ...
                  <StaggerItem key={post.slug}>
                    <article className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg p-6 hover:border-purple-500 transition-colors group">
                      <div className="flex items-center justify-between text-sm text-gray-400 mb-3">
                        ...
                        <span>
                          {post.readingTime} •{" "}
                          {post.wordCount.toLocaleString(locale === "fr" ? "fr-FR" : "en-US")} words
                        </span>
                      </div>

                      <h2 className="text-xl font-bold text-white mb-3 group-hover:text-accent transition-colors">
                        <NextLink href={`/${locale}/blog/${post.slug}`}>{post.title}</NextLink>
                      </h2>
                      ...
                      >
                        ...
                        <svg
                          ...
                          viewBox="0 0 24 24"
                          role="img"
                          aria-labelledby={readArrowTitleId}
                        >
                          <title id={readArrowTitleId}>Read more</title>
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            strokeWidth={2}
