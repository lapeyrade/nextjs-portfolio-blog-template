...
const rateLimits: Map<string, { count: number; windowStart: number }> =
	new Map();
...
function validatePayload(data: unknown): {
	...
	const value =
		typeof d.value === "number" && Number.isFinite(d.value)
			? d.value
			: undefined;
	if (value !== undefined && (value < -1e6 || value > 1e8))
		return { ok: false, error: "Metric value out of range" };
	...
	const now = Date.now();
	if (Math.abs(now - timestamp) > 7 * 24 * 60 * 60 * 1000)
		return { ok: false, error: "Timestamp out of acceptable range" };

	// Handle WebVitals component field names
	const url =
		typeof d.url === "string"
			? d.url.trim().slice(0, 2000)
			: typeof d.path === "string"
				? d.path.trim().slice(0, 2000)
				: undefined;

	const userAgent =
		typeof d.userAgent === "string"
			? String(d.userAgent).slice(0, 512)
			: typeof d.ua_snippet === "string"
				? String(d.ua_snippet).slice(0, 512)
				: undefined;

	const id = typeof d.id === "string" ? String(d.id).slice(0, 256) : undefined;
	const label =
		typeof d.label === "string" ? String(d.label).slice(0, 64) : undefined;
	const delta =
...
export async function POST(request: Request) {
	try {
		...
		for (const metric of metrics) {
			...
			if (!ok) {
				return NextResponse.json(
					{ ok: false, error: `Invalid metric: ${error}` },
					{ status: 400 },
				);
			}
			if (payload) {
...
export async function GET() {
	return NextResponse.json({
		ok: true,
		message: "POST Web Vitals metrics to this endpoint.",
	});
}
