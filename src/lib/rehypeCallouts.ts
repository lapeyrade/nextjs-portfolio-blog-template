...
export default function rehypeCallouts() {
    return function transform(tree: HastNode) {
        function walk(node: HastNode) {
            if (isElement(node, 'blockquote') && Array.isArray(node.children) && node.children.length > 0) {
                const first = node.children[0]
                if (isElement(first, 'p') && Array.isArray(first.children) && first.children.length > 0) {
                    const firstChild = first.children[0]
                    if (firstChild && firstChild.type === 'text' && typeof firstChild.value === 'string') {
                        const match = firstChild.value.match(/^\s*\[!(NOTE|TIP|WARNING|IMPORTANT|CAUTION|INFO)\]\s*/)
                        if (match) {
                            const kind = TYPE_MAP[match[1] as keyof typeof TYPE_MAP] || 'note'
                            // Remove the marker from the text
                            firstChild.value = firstChild.value.replace(match[0], '')
                            node.properties = node.properties || {}
                            const classNames = ensureArrayClassName(node.properties)
                            node.properties.className = Array.from(new Set([...classNames, 'callout', `callout-${kind}`]))
                        }
                    }
                }
            }
